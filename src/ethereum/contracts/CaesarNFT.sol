...
contract CaesarNFT is ERC721Enumerable, Ownable {
    ...
    function reserveCaesar(address _to, uint256 _amount) external onlyOwner() {
    ...
    function claimCaesar(uint numberOfTokens) public {
        uint256 supply = totalSupply();
        require(numberOfTokens <= 4, "You can mint max 4 tokens at a time");
        require(totalSupply() + numberOfTokens <= (caesarSupply - reserve), "Exceeds max supply");
